...
def generate_signals(data):
    ...
    ema_cross_up = (data['EMA50'] > data['EMA200']) & (data['EMA50'].shift(1) <= data['EMA200'].shift(1))
    ema_cross_down = (data['EMA50'] < data['EMA200']) & (data['EMA50'].shift(1) >= data['EMA200'].shift(1))
    
    macd_cross_up = (data['MACD'] > data['Signal_Line']) & (data['MACD'].shift(1) <= data['Signal_Line'].shift(1))
    macd_cross_down = (data['MACD'] < data['Signal_Line']) & (data['MACD'].shift(1) >= data['Signal_Line'].shift(1))
    
    rsi_oversold = data['RSI'] < 30
    rsi_overbought = data['RSI'] > 70
    
    price_below_lower = data['Close'] < data['Lower_Band']
    price_above_upper = data['Close'] > data['Upper_Band']
    
    # Generate signals
    buy_signals = ema_cross_up & macd_cross_up & rsi_oversold & price_below_lower
    sell_signals = ema_cross_down & macd_cross_down & rsi_overbought & price_above_upper
